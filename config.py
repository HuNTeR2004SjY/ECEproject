...
# Using Groq's Llama model for better reasoning capabilities
GENERATOR_MODEL = os.getenv('ECE_GENERATOR_MODEL', 'llama-3.3-70b-versatile')

# Generation parameters (optimized for Llama 3.3 70B)
GENERATION = {
    'max_input_length': 8192,       # Llama 3 has a huge context window
    'max_output_length': 1024,      # Need higher output lengths for steps
    'min_output_length': 60,
    'temperature': 0.3,             # Lower temperature for precise support steps
    'top_p': 0.9,                   # Good defaults for clear text
    'presence_penalty': 0.1,        # Slight penalty to keep instructions flowing smoothly
}
...
GENAI_EMAIL_MODEL = os.getenv('ECE_GENAI_EMAIL_MODEL', 'gemini-1.5-flash')
...
JIRA_TYPE_MAP = {
    'Incident':        'Bug',
    'Problem':         'Bug',
    'Change Request':  'Story', # Changed from 'Task' to 'Story'
    'Bug':             'Bug',   # Added
    'Task':            'Task',  # Added
...
SLACK = {
    ...
    # Channel names — these must exist in your Slack workspace
    'channels': {
        'it_support':   os.getenv('SLACK_CHANNEL_IT',        'it-support'),
        'escalations':  os.getenv('SLACK_CHANNEL_ESCALATIONS','it-escalations'),
        'incidents':    os.getenv('SLACK_CHANNEL_INCIDENTS',  'incidents'),
        'logs':         os.getenv('SLACK_CHANNEL_LOGS',       'ece-logs'),
    },

    # Map ECE queue names → Slack channel keys
    'queue_channel_map': {
        'IT Support':      'it_support',
        'Technical Support':'it_support',
        'Engineering':     'it_support',
        'HR':              'logs',
        'Billing':         'logs',
    },
}
...
# Add to .env alongside existing keys:
#   SLACK_BOT_TOKEN=xoxb-...              (same value as used above in SLACK)
